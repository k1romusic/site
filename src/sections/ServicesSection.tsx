...
import { Sliders, Music, Mic2, Cpu, CheckCircle2, ArrowUpRight, Sparkles, Disc } from 'lucide-react';
    ...
    <section id="services" className="relative pt-6 pb-16 sm:pb-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 sm:mb-12 gap-6">
        ...
        <p className="text-sm sm:text-base text-[#F5F0E8]/70 max-w-lg leading-relaxed">
          {lang === 'ru'
            ? 'Полный спектр студийных решений от прецизионного сведения и песен вашим голосом до авторского софта.'
            : 'End-to-end studio solutions from high-end mixing and neural vocal production to proprietary software.'}
        </p>
      </div>

      {/* Modern 5-Service Grid */}
        ...
        {servicesList.map((service) => {
          ...
          return (
            <a
              key={service.id}
              href={cardLink}
              className={`block group ${isAiTools ? 'sm:col-span-2 lg:col-span-2' : ''}`}
            >
              <GlassCard
                hoverEffect={true}
                glowOnHover={true}
